...
/**
 * List<String> 的 MyBatis 类型处理器，将 List<String> 转换为数据库的 VARCHAR 类型。
 *
 * <p>
 * 使用逗号分隔符将字符串列表存储为字符串，并在读取时解析回 List<String>。
 * </p>
 *
 * @author JCodeNest
 * @version 1.0.0
...
public class StringListTypeHandler implements TypeHandler<List<String>> {
    ...
    /**
     * 将 List<String> 转换为逗号分隔的字符串并设置到数据库。
     *
     * @param ps        预处理语句
     * @param i         参数索引
    ...
    public void setParameter(PreparedStatement ps, int i, List<String> parameter, JdbcType jdbcType) throws SQLException {
        ...
        String value = parameter == null || parameter.isEmpty() ? null : String.join(COMMA, parameter);
        ps.setString(i, value);
    }

    /**
     * 从结果集中获取指定列名的值并解析为 List<String>。
     *
     * @param rs         结果集
     * @param columnName 列名
     * @return 字符串列表
    ...
    /**
     * 从结果集中获取指定列索引的值并解析为 List<String>。
     *
     * @param rs          结果集
    ...
    /**
     * 从存储过程中获取指定列索引的值并解析为 List<String>。
     *
     * @param cs          存储过程调用语句
     * @param columnIndex 列索引
     * @return 字符串列表
