...
/**
 * List<Long> 的 MyBatis 类型处理器，将 List<Long> 转换为数据库的 VARCHAR 类型。
 *
 * <p>
 * 使用逗号分隔符将长整型列表存储为字符串，并在读取时解析回 List<Long>。
 * </p>
 *
 * @author JCodeNest
 * @version 1.0.0
...
public class LongListTypeHandler implements TypeHandler<List<Long>> {
    ...
    /**
     * 将 List<Long> 转换为逗号分隔的字符串并设置到数据库。
     *
     * @param ps        预处理语句
     * @param i         参数索引
     * @param parameter 长整型列表
    ...
    /**
     * 将逗号分隔的字符串解析为 List<Long>。
     *
     * @param value 逗号分隔的字符串
     * @return 长整型列表，如果输入为空或无效则返回空列表
     */
